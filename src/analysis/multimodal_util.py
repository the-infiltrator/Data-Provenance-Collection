...
def classify_and_resolve_licenses(
    ...
    for (license_name, license_url) in license_infos:
        ...
        classifications = filters.classify_license(
            license_name, license_url, all_constants)
...
def map_license_criteria_multimodal(
    ...
    for row in data_summary:
        ...
        for license_info in row["Licenses"]:
            ...
            if license_info["License"] != "OpenAI":
                our_uid_to_license_infos_no_openai[uid].append(
    ...
    for uid in our_uid_to_license_infos.keys():
        ours_resolved[uid] = classify_and_resolve_licenses(
            our_uid_to_license_infos[uid], all_constants)
        ours_openai_resolved[uid] = classify_and_resolve_licenses(
            our_uid_to_license_infos_no_openai[uid], all_constants)

    data_summary = add_license_classes_to_summaries(
        data_summary, ours_resolved, "DataProvenance")
    data_summary = add_license_classes_to_summaries(
        data_summary, ours_openai_resolved, "DataProvenance IgnoreOpenAI")
...
african_continent_iso_codes = [12, 24, 204, 72, 86, 854, 108, 132, 120, 140, 148, 174, 178, 180, 384, 262, 818, 226, 232, 748, 231, 260, 266, 270, 288, 324, 624,
                               404, 426, 430, 434, 450, 454, 466, 478, 480, 175, 504, 508, 516, 562, 566, 638, 646, 654, 678, 686, 690, 694, 706, 710, 728, 729, 834, 768, 788, 800, 732, 894, 716]
...
def read_continent_country_iso_codes():
    ...
    try:
        ...
        logging.error(
            "The file '../../constants/continent_country_iso.json' was not found.")
        return []
    except json.JSONDecodeError:
        logging.error(
            "Error decoding JSON from the file '../../constants/continent_country_iso.json'.")
        return []
    ...
    for continent_data in data['continents']:
        ...
        for country_data in countries:
            ...
            continent_country_iso_list.append(
                {'continent': continent, 'country': country, 'iso_code': iso_numeric})
...
def get_continent(x: str, continent_country_iso_list: list) -> typing.List[int]:
    ...
    for country in x:
        ...
        filtered_country = df_continent_country_iso[df_continent_country_iso['country'].str.lower(
        ) == country.lower()]

        if len(filtered_country) > 0:
            continent_set.update(
                filtered_country['continent'].unique().tolist())
        else:
            logging.warning(f"Country '{country}' not found in the data.")
...
def get_continent_id(x: str, continent_country_iso_list: list) -> typing.List[int]:
    ...
    df_continent_country_iso = pd.DataFrame(continent_country_iso_list)
    continent_iso_ids = df_continent_country_iso[df_continent_country_iso['continent'] == x]['iso_code'].apply(
        int).unique().tolist()
...
def plot_license_terms_stacked_bar_chart(
    ...
    if save_dir:
        chart_licenses.save(os.path.join(
            save_dir, "license_use_by_modality.png"), ppi=plot_ppi)
...
def prep_summaries_for_visualization(
    ...
    license_paraphrases = invert_dict_of_lists(
        all_constants["LICENSE_PARAPHRASES"])
    creator_groupmap = invert_dict_of_lists(all_constants["CREATOR_GROUPS"])
    creator_countrymap = invert_dict_of_lists(
        all_constants["CREATOR_COUNTRY_GROUPS"])
    ...
    df_text["Data Terms"] = df_text["Collection"].apply(
        lambda x: collection_to_terms_mapper[x] if x in collection_to_terms_mapper else None
    )

    df_text["Language Families"] = df_text["Languages"].map(
        lambda c: [lang_typmap[ci] for ci in c])

    df_speech = pd.DataFrame(speech_summaries).assign(
        Modality="Speech").rename(columns={"Location": "Countries"})
    df_speech["Data Terms"] = df_speech["Collection"].apply(
        lambda x: collection_to_terms_mapper[x] if x in collection_to_terms_mapper else None
    )
    ...
    df_video["Data Terms"] = df_video["Dataset Name"].apply(
        lambda x: collection_to_terms_mapper[x] if x in collection_to_terms_mapper else None
    )

    df_text["Year Released"] = df_text["Inferred Metadata"].map(
        get_year_for_text)
    # Combine modalities
    ...
    df["Creator Categories"] = df["Creators"].map(
        lambda c: [creator_groupmap[ci] for ci in c])
...
def plot_stacked_temporal_license_categories(
    ...
    if save_dir:
        chart_licensesyears.save(os.path.join(
            save_dir, "license_use_by_modality+year.png"), ppi=plot_ppi)
...
def categorize_creators(df, order):
    ...
    df_categories["Creator Categories"] = df_categories["Creator Categories"].fillna(
        "Unspecified")
...
def categorize_sources(df, order, domain_typemap):
    ...
    df_sources["Source Category"] = df_sources.apply(
        map_domaingroup, axis=1).fillna("Other")
...
def plot_stacked_creator_categories(
    ...
    if collection_level:
        def unpack_list(cats):
            ...
            flatten_cats = list(
                set([cat for catsll in cat_list for cat in catsll]))
            # print(flatten_cats)
    ...
    if save_dir:
        chart_categories.save(os.path.join(
            save_dir, "creator_categories_by_modality.png"), ppi=300)
...
def plot_donut_creator_categories(
    ...
    if save_dir:
        chart_categoriesalt.save(os.path.join(
            save_dir, "creator_categories_by_modality-alt.png"), ppi=300)
...
def plot_stacked_temporal_creator_categories(
    ...
    if save_dir:
        chart_categoriesyears.save(os.path.join(
            save_dir, "creator_categories_by_modality+year.png"), ppi=300)
...
def plot_altair_worldmap_country(
    ...
    df_countries = df.explode("Countries").dropna(
        subset=["Countries"])  # Drop rows with no country for the moment
    df_countries = df_countries[["Countries", "Modality"]
                                ].value_counts().reset_index(name="Count")
    df_countries["Country ID"] = df_countries["Countries"].map(get_country)
    # If couldn't be found (see any logged warnings), drop it
    df_countries = df_countries.explode(
        "Country ID").dropna(subset=["Country ID"])
    ...
    # Loop through modalities and create individual charts
    for modality, color in modality_colors.items():
        ...
        chart = base.encode(
            ...
            # Dynamic title that includes the Gini score
            title=f"{modality} (Gini={gini_scores.get(modality, 'N/A')})"
        )

        # Save the individual chart as high-resolution PNG
...
def crop_and_combine_images(image_paths, save_dir, output_filename, crop_percentage):
    """Crops each image on the left and right and combines them into a single horizontal image."""
    images = [Image.open(image) for image in image_paths]

    cropped_images = []
    for img in images:
        # Crop a percentage of the image width from the left and right
        width, height = img.size
        left_crop = int(width * crop_percentage)
        right_crop = width - left_crop
        cropped_img = img.crop((left_crop, 0, right_crop, height))
        cropped_images.append(cropped_img)

    # Get the new width and height of cropped images
    widths, heights = zip(*(i.size for i in cropped_images))

    # Total width is the sum of individual widths, height is the max height
    total_width = sum(widths)
    max_height = max(heights)

    # Create a blank canvas with the combined size
    combined_image = Image.new('RGB', (total_width, max_height))

    # Paste each cropped image side by side
    x_offset = 0
...
def plot_altair_worldmap_continent(
    ...
    df_countries = df
    df_countries["Continent"] = df_countries["Countries"].map(
        lambda x: get_continent(x, continent_country_iso_list))
    df_countries = df_countries.explode(
        "Continent").dropna(subset=["Continent"])
    # df_countries.to_csv('df_continent.csv')
    df_countries = df_countries[["Continent", "Modality"]
                                ].value_counts().reset_index(name="Count")
    df_countries["Continent ISO ID"] = df_countries["Continent"].map(
        lambda x: get_continent_id(x, continent_country_iso_list))
    df_countries = df_countries.explode("Continent ISO ID").dropna(
        # If couldn't be found (see any logged warnings), drop it
        subset=["Continent ISO ID"])
    ...
    for modality, color in modality_colors.items():
        ...
        chart = base.encode(
            ...
            lookup="id",
            from_=alt.LookupData(modality_data, "Continent ISO ID", [
                                 "Count", "Modality", "Continent"])
        ).transform_calculate(
            Count="isValid(datum.Count) ? datum.Count : 0",
            Modality="isValid(datum.Modality) ? datum.Modality : ''",
    ...
    if save_dir:
        chart_map.save(os.path.join(
            save_dir, "dataset_count_by_continent_and_modality.png"), ppi=300)
...
def plot_source_domain_stacked_chart(
    ...
    if save_dir:
        chart_sources.save(os.path.join(
            save_dir, "source_categories_by_modality.png"), ppi=300)
...
def plot_stacked_temporal_source_categories(
    ...
    if save_dir:
        chart_sourcesyears.save(os.path.join(
            save_dir, "source_categories_by_modality+year.png"), ppi=300)
...
def text_groupby_collection(df, mode_column, fn):
    ...
    df_text.loc[:, mode_column] = df_text.groupby(
        "Collection")[mode_column].transform(fn)
...
def plot_source_domain_stacked_chart_collections(
    ...
    logging.warning("Aggregating to %d collections" % len(
        df_sources.loc[df_sources["Modality"] == "Text", "Collection"].unique()))
    ...
    if save_dir:
        chart_sourcesaggregated.save(os.path.join(
            save_dir, "source_categories_by_modality-aggregated.png"), ppi=300)
...
def plot_license_terms_stacked_bar_chart_collections(
    ...
    chart = alt.Chart(df).mark_bar().encode(
        ...
        color=alt.Color(
            f"{license_key}:N",
            # Map each license to a specific color
            scale=alt.Scale(domain=license_order, range=license_palette),
            title=license_key,
            sort=license_order,
        ),
        # Ensures correct order of the bars
    ...
    if save_dir:
        chart.save(os.path.join(
            save_dir, "license_use_by_modality_collections.png"), ppi=plot_ppi)
...
def order_by_grouped_permisiveness(
    ...
    permisiveness_order = permisiveness.sort_values(
        by="Permissiveness")[group_column].tolist()
...
def generate_multimodal_license_terms_latex(df):
    dfx = df.groupby(['Modality', 'License | Terms']
                     ).size().reset_index(name='counts')

    # Apply the function and assign the results to new columns
    dfx['License Info'] = dfx['License | Terms'].apply(
        lambda x: x.split(' | ')[0].strip())
    dfx['Terms Info'] = dfx['License | Terms'].apply(
    ...
    for modality in modalities:
        ...
        counts_dict = {license: {term: 0 for term in terms_info}
        ...
        for license in counts_dict:
            for term in counts_dict[license]:
                counts_dict[license][term] = round(
                    (counts_dict[license][term] / total_counts) * 100, 1)

        # Generate LaTeX table
        latex_table = "\\begin{table*}[t!]\n\\centering\n\\begin{adjustbox}{width=0.98\\textwidth}\n"
        latex_table += "\\begin{tabular}{l|" + "r" * \
            len(terms_info) + "|r}\n\\toprule\n"
        joined_col_headers = " & ".join(
            ["\\textsc{" + ti + "}" for ti in terms_info])
        latex_table += "\\textsc{License / Terms} & " + \
            joined_col_headers + " & \\textsc{Total} \\\\\n"
        latex_table += "\\midrule\n"
        for license in counts_dict:
            ...
            if license == "Total":
                latex_table += "\\midrule\n"
            latex_table += "\\textsc{" + license + "} & " + \
                " & ".join(lic_vals) + " & " + total_vals + " \\\\\n"
        mod_label = modality.lower() + "_license_terms_breakdown"
        unspec_or_open = counts_dict["Commercial"]["Unrestricted"] + counts_dict["Commercial"]["Unspecified"] + \
            counts_dict["Unspecified"]["Unrestricted"] + \
            counts_dict["Unspecified"]["Unspecified"]
        closed_pct = round(100 - unspec_or_open, 1)
        total_nc_license = round(counts_dict["NC/Acad"]["Total"], 1)
        total_restrictive_terms = round(
...
def categorize_tasks(df, order, domain_typemap, tasks_column, modality, collections_datasets_flag):
    def map_taskgroup(row, modality) -> str:
        ...
        # If the modality is "Speech", return the task without domain mapping
            ...
            logging.warning(
                "Unsupported modality for task categorization: %s" % modality)
            return "Other"
    ...
    if modality == "Text":
        if collections_datasets_flag == 'Datasets':
            ...
            df_agg = df.groupby('Collection')[tasks_column].apply(
                lambda x: list(set.union(*map(set, x)))).reset_index()
    ...
    df_tasks[tasks_column] = df_tasks.apply(
        lambda row: map_taskgroup(row, modality), axis=1).fillna("Other")
...
def plot_tasks_chart(
    ...
    df_sources = categorize_tasks(
        df, order, task_typemap, tasks_column, modality, collections_datasets_flag)
    ...
    df_datasets_aggregated = df_sources['Dataset Name'].value_counts(
    ).reset_index()
    ...
    df_tasks_aggregated['percentage'] = (
        (df_tasks_aggregated['count'] / df_datasets_aggregated['count'].sum()) * 100).round().astype(int)

    # Sort the tasks based on the count
    sorted_order = df_tasks_aggregated.sort_values('count', ascending=False)[
        tasks_column].tolist()

    # Assign a position to each task based on the sorted order
    df_tasks_aggregated['position'] = df_tasks_aggregated[tasks_column].apply(
        lambda x: sorted_order.index(x))

    # Assign a color to each task based on the position
    color_scale = alt.Scale(domain=list(
        range(len(sorted_order))), scheme='tableau20')
    ...
    bar_chart = alt.Chart(df_tasks_aggregated).mark_bar(size=12).encode(
        ...
        x=alt.X(
            ...
            axis=alt.Axis(format='.0f', labelExpr="datum.value + '%'",
                          labelFontSize=font_size, titleFontSize=font_size+2)  # format percentage
        ),
        color=alt.Color(
            field="position",
            type="ordinal",
...
def tokens_calculation(df):
    ...
    df = df.copy()
    text_metrics = df[['Unique Dataset Identifier', 'Text Metrics']].groupby(
        'Unique Dataset Identifier').first().reset_index()
    ...
    metrics_df['Total Tokens'] = metrics_df.apply(lambda row: (
        row['Num Dialogs'] * row['Mean Dialog Turns'] *
        (row['Mean Inputs Length'] + row['Mean Targets Length'])
    ), axis=1)

    # Select relevant columns and merge with original DataFrame
    df_token = metrics_df[['Unique Dataset Identifier', 'Total Tokens']]
    tokens_output = pd.merge(
        df, df_token, on='Unique Dataset Identifier', how='left')
...
def chart_creation(
    ...
    chart1 = alt.Chart(df).mark_bar(color=color).encode(
        ...
        y=alt.Y(f'{x_field}:N', sort=labels, axis=alt.Axis(
            title=None, grid=False, domain=False, ticks=False)),
        x=alt.X('Number of datasets:Q', scale=alt.Scale(domain=[
                0, max_count * ratio]), axis=alt.Axis(grid=False, domain=False, tickCount=5)),
...
def combined_dim_charts(*charts):
    ...
    combined_chart = alt.hconcat(*charts, spacing=1).resolve_scale(
        ...
        # Ensures independent shape scale for each chart (if applicable)
        shape="independent",
        # Ensures independent size scale for each chart (if applicable)
        size="independent"
    ).resolve_legend(
        color="independent",  # Ensures each chart has an independent legend for color
        # Ensures each chart has an independent legend for shape (if applicable)
        shape="independent",
        # Ensures each chart has an independent legend for size (if applicable)
        size="independent"
    ).configure(
...
def data_aggregation_for_chart(df, modality: str, bins, labels,  measure_column: str, group_column: str, by_collection=False):
    ...
    if by_collection:
        ...
        df_modality = df_modality.groupby(
            'Collection')[measure_column].sum().reset_index()
        df_modality[group_column] = pd.cut(
            df_modality[measure_column], bins=bins, labels=labels, right=False)
    else:
...
def plot_temporal_cumulative_sources(
    ...
    def prep_df(df, modality, top_n, cumulative_measurement, earliest_year=2015):
        ...
        df_modsourceyears = reduce_categories_to_topk(
            df_modsourceyears, "Source Category", top_n)
        df_modsourceyears['Source Category'] = df_modsourceyears['Source Category'].apply(
            lambda x: x.title())
        ...
        df_modsourceyears['Source Category'].replace(
            source_cat_mapper, inplace=True)
        if earliest_year > 2013:
            rep_map = {str(yr): f"<{earliest_year}" for yr in range(
                2013, earliest_year)}
        ...
        df_modsourcecumulativeyears = df_modsourcecumulativeyears.sort_values(
            by="Year Released")
        # Assuming your dataframe is named df
        df_modsourcecumulativeyears = df_modsourcecumulativeyears[
            df_modsourcecumulativeyears['Year Released'] != "Unknown"]
        df_modsourcecumulativeyears = df_modsourcecumulativeyears[
            df_modsourcecumulativeyears['Source Category'] != "Unsure"]
        # print(df_modsourcecumulativeyears)
        return df_modsourcecumulativeyears[["Year Released", "Source Category", "Cumulative Hours"]]
    df_cumyears = prep_df(df, modality, top_n,
                          cumulative_measurement, earliest_year=earliest_year)
    # return df_cumyears

    df_final_values = df_cumyears.groupby('Source Category').apply(
        lambda x: x.loc[x['Year Released'].idxmax()])
    df_final_values = df_final_values[['Source Category', 'Cumulative Hours']].sort_values(
        'Cumulative Hours', ascending=False)
    # Use the sorted categories for the legend order
    sorted_categories = df_final_values['Source Category'].tolist()
    # print(sorted_categories)
    # print(df_cumyears)

    YEARS_ORDER = [f"<{earliest_year}"] + \
        [str(year) for year in range(earliest_year, 2025)]
    ...
    chart_sourceyearhours = alt.Chart(
        ...
        color=alt.Color(
            ...
            legend=alt.Legend(
                ...
                # Wrap every K entries (replace K with the number of entries per row)
                columns=4
            ),
    ...
    if save_png:
        chart_sourceyearhours.save(
            os.path.join(
                PLOT_DIR, f"{modality}_sourcecategories-cumulativehours.png"),
            ppi=PLOT_PPI
        )
...
def extract_lang_fam_mappers():
    ...
    df_langsglottolog = pd.read_csv(
        "data/speech_supporting_data/languages.csv")
...
def get_langfamily(
    ...
    try:
        ...
        lang = langcodes.get(langcodes.standardize_tag(
            lang, macro=True)).to_alpha3()
...
def get_hours_for_dataset_and_language(row: pd.Series) -> float:
    df_yodashours = pd.read_csv(
        "data/speech_supporting_data/yodas_splithours.csv").rename({"hours": "Hours"}, axis=1)
    df_yodashours["Language (ISO)"] = df_yodashours["split"].map(
        lambda x: x[:2])
    df_yodashours["Language (Name)"] = df_yodashours["Language (ISO)"].map(
        lambda x: langcodes.Language.make(
            language=langcodes.standardize_tag(x, macro=True)).language_name()
    )
    df_commonvoicehours = pd.read_json(
        "data/speech_supporting_data/commonvoice_splithours.json").T
    ...
    for langcode_simplified, langcode_data_to_aggregate in language_codes_to_aggregate.items():
        df_commonvoicehours.loc[langcode_simplified,
                                "total_clips_duration"] = df_commonvoicehours.loc[langcode_data_to_aggregate, "total_clips_duration"].sum()

    df_commonvoicehours["Hours"] = df_commonvoicehours["total_clips_duration"] / 60 / 60 / 1000
    df_commonvoicehours = df_commonvoicehours.rename(
        columns={'Languages (ISO)': 'Language (ISO)'})
    df_commonvoicehours = df_commonvoicehours.reset_index(
        names=["Language (ISO)"])

    df_multilinguallibrispeechhours = pd.read_csv(
        "data/speech_supporting_data/multilinguallibrispeech_splithours.csv")
    df_multilinguallibrispeechhours["Hours"] = df_multilinguallibrispeechhours[df_multilinguallibrispeechhours.columns[1:]].sum(
        axis=1)
    df_multilinguallibrispeechhours["Language (ISO)"] = df_multilinguallibrispeechhours.language.map(
        lambda x: langcodes.find(x).to_tag())

    df_bloomspeechhours = pd.read_csv(
        "data/speech_supporting_data/bloomspeech_splithours.csv")
    df_bloomspeechhours["Hours"] = df_bloomspeechhours[df_bloomspeechhours.columns[2:]].sum(
        axis=1) / 60
    df_bloomspeechhours["Language (ISO)"] = df_bloomspeechhours["ISO-639-3"].map(
        lambda x: langcodes.standardize_tag(x, macro=True))

    df_fleursspeechhours = pd.read_csv(
        "data/speech_supporting_data/fleurs_splithours.csv")
    df_fleursspeechhours = df_fleursspeechhours.rename(
        columns={'Languages (ISO)': 'Language (ISO)'})
    ...
    if dataset in dataset_hoursmapping:
        ...
        if hours == 0:
            print("Hours not found for language code %s in dataset %s" %
                  (language, dataset))
        return hours
...
def compute_temporal_gini_bounds(df_spec, measure_key, cumulation_key):
    ...
    df_spec_cum_gini["Gini Mean"] = df_spec_cum_gini["Gini"].map(
        lambda x: x[0])
    df_spec_cum_gini["Gini Lower"] = df_spec_cum_gini["Gini"].map(
        lambda x: max(0, x[1]))
    df_spec_cum_gini["Gini Upper"] = df_spec_cum_gini["Gini"].map(
        lambda x: min(x[2], 1))
...
def prepare_speech_for_gini(df):
    ...
    df_speechlanguagenames["Language (Name)"] = df_speechlanguagenames["Language (ISO)"].map(
        lambda x: langcodes.Language.make(language=langcodes.standardize_tag(
            x.split("-")[0].split("_")[0], macro=True)).language_name()
    )
    df_speechlanguagesn = df_speechlanguagenames.copy()

    df_speechlanguagesn["Language (ISO)"] = df_speechlanguagesn["Language (ISO)"].map(
        lambda x: x.split("_")[0].split("-")[0])
    lang_fam_infos = extract_lang_fam_mappers()
    df_speechlanguagesn["Language Family"] = df_speechlanguagesn["Language (ISO)"].map(
        lambda x: get_langfamily(x, lang_fam_infos, []))
    ...
    df_speechlanguagesn["Hours"] = df_speechlanguagesn.apply(
        get_hours_for_dataset_and_language, axis=1)
    ...
    speech_df_spec_cum_gini_langs = compute_temporal_gini_bounds(
        df_speechlanguagesn, "Language (ISO)", "Hours")
    speech_df_spec_cum_gini_langfams = compute_temporal_gini_bounds(
        df_speechlanguagesn, "Language Family", "Hours")
...
def prep_text_for_lang_gini(df, all_constants):
    ...
    # [["Year Released", "Total Tokens", "Languages", "Language Families"]]
    ...
    df_text_lang_explode["Language Families"] = df_text_lang_explode["Languages"].map(
        lambda c: LANG_GROUP_MAPPER[c])
    ...
    df_text_lang_explode = df_text_lang_explode[[
        "Collection", "Dataset Name", "Year Released", "Tokens", "Languages", "Language Families"]]
    ...
    df_text_lang_explode['Language (ISO)'] = df_text_lang_explode['Languages'].map(
        lang_iso_mapper)
    ...
    df_text_lang_explode["Language Family"] = df_text_lang_explode["Language (ISO)"].map(
        lambda x: get_langfamily(x, lang_fam_infos, code_languages))

    # df_text = df_text.rename(columns={'Languages': 'Language (ISO)', "Language Families": "Language Family"})

    text_df_spec_cum_gini_langs = compute_temporal_gini_bounds(
        df_text_lang_explode, "Language (ISO)", "Tokens")
    text_df_spec_cum_gini_langfams = compute_temporal_gini_bounds(
        df_text_lang_explode, "Language Family", "Tokens")
...
def prepare_geo_gini_data(df):
    ...
    df_spec_locs = pd.concat([df_text_locs, df_speech_locs, df_video_locs])[
        ["Unique Dataset Identifier", "Countries", "Dimension", "Modality", "Year Released"]]

    text_df_spec_cum_gini_locs = compute_temporal_gini_bounds(
        df_text_locs, "Countries", "Dimension")
    text_df_spec_cum_gini_locs["Type"] = "Text"
    speech_df_spec_cum_gini_locs = compute_temporal_gini_bounds(
        df_speech_locs, "Countries", "Dimension")
    speech_df_spec_cum_gini_locs["Type"] = "Speech"
    video_df_spec_cum_gini_locs = compute_temporal_gini_bounds(
        df_video_locs, "Countries", "Dimension")
    video_df_spec_cum_gini_locs["Type"] = "Video"
    df_gini_locs = pd.concat(
        [text_df_spec_cum_gini_locs, speech_df_spec_cum_gini_locs,
            video_df_spec_cum_gini_locs]
    )
...
def plot_cum_barchart(df_spec_src, target):
    ...
    chart = alt.Chart(df_spec_src).mark_bar(
        ...
        # Sorted as requested
        xOffset=alt.XOffset("Modality:N", sort=["Video", "Speech", "Text"]),
